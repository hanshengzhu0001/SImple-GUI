...
let with_thickness (g: gctx) (th: int) : gctx =
  { g with thickness = th }
...
let draw_rect (g: gctx) (p1: position) ((w, h): dimension) : unit =
  set_graphics_state g;
  let (x, y) = ocaml_coords g p1 in (*upper-left*)
  let bottomleft_y = y-h in
  Graphics.draw_rect x bottomleft_y w h 
...
let draw_ellipse (g: gctx) (p: position) (rx: int) (ry: int) : unit =
  set_graphics_state g;
  let (x, y) = ocaml_coords g p in (*center*)
  Graphics.draw_ellipse x y rx ry

(*Is p the upper-left corner or the center of the bounding box*)
