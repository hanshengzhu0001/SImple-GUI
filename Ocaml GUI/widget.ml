...
let vpair (w1: widget) (w2: widget) : widget = {
    repaint = (fun (g: Gctx.gctx) ->
      ...
      (*repaint at a new position, aligned horizontally*)

    handle = (fun (g:Gctx.gctx) (e:Gctx.event) ->
        if event_within g e (w1.size()) 
        (*determines if the given event is within the region*)
        then w1.handle g e
        else
          let g2 = (Gctx.translate g (0, snd (w1.size ()))) in 
          (*returns the height of w1.size()*)
          if event_within g2 e (w2.size ())
          then w2.handle g2 e
          else ());
    ...
    (*takes the maximum of x1 and x2 to get the total width*)
}
...
let list_layout (pair: widget -> widget -> widget)
  ...
  begin match ws with
  |[] -> empty_widget
  |first::rest -> List.fold_left pair first rest
  (*pair each widget from left to right*)
  end

let hlist (ws: widget list) : widget = 
  begin match ws with
  |[] -> empty_widget
  |first::rest -> List.fold_left hpair first rest
  end

let vlist (ws: widget list) : widget =
  begin match ws with
  |[] -> empty_widget
  |first::rest -> List.fold_left vpair first rest
  end
...
let make_controller (v: 'a) : 'a value_controller =
  let state = {contents = (v, [])} in
  (*need a value and a list of listeners*)
  let add_change_listener listener = 
  (*listener has type 'a -> unit*)
  let (value,listeners) = state.contents in
  state.contents <- (value, listener::listeners)
  in

  let get_value () =
  let (value,_) = state.contents in
  value
  in

  let change_value new_value =
  let (_,listeners) = state.contents in
  state.contents <- (new_value, listeners);
  List.iter (fun listener -> listener new_value) listeners
  in

  {add_change_listener; get_value; change_value}
...
let checkbox (init: bool) (s: string) : widget * bool value_controller =
  let bc = make_controller init in
  let repaint g = 
      let is_checked = bc.get_value () in
      Gctx.draw_rect g (5,5) (15,15);
      Gctx.draw_string g (22,5) s;
      if is_checked then 
      (Gctx.draw_rect g (5,5) (15,15);
       Gctx.draw_line g (5,5) (20,20);
       Gctx.draw_line g (20,5) (5,20))
      in
  let handle g e = 
      begin match Gctx.event_type e with
      |Gctx.MouseDown -> update_value bc (fun x -> not x)
      |_ -> ()
      end
      in
  let size sz =
      let width = 20 + fst(Gctx.text_size s) in
      let height = 10 + snd(Gctx.text_size s) in
      (width,height) in

      let w = {repaint; handle; size} in
      (w,bc)
...
let slider (init_value: int) (s: string) : 
                  widget * int value_controller =
  let slider_controller = make_controller init_value in
  ...
  let slider_widget =
    {
      repaint = (fun (gc: Gctx.gctx) ->
        (* Set the slider bar's background green*)
        Gctx.fill_rect (Gctx.with_color gc Gctx.green) (0, 0) (width, height);

        (* Draw the thumb as a yellow rectangle *)
        let thumb_width = 5 in
        let thumb_x = ((slider_controller.get_value () - 1) * 
        (width - thumb_width)) / 4 in (*scaling and draw the location 
                                        of the thumb*)
        let thumb_position = (thumb_x, 0) in
        let thumb_size = (thumb_width, height) in
        Gctx.fill_rect (Gctx.with_color gc Gctx.yellow) 
        thumb_position thumb_size;
      );
      handle = (fun (gc: Gctx.gctx) (event: Gctx.event) ->
        match Gctx.event_type event with
        | Gctx.MouseDown 
        | Gctx.MouseDrag ->
          let pos = Gctx.event_pos event gc in
          let thumb_width = 5 in
          let new_thumb_x = max 0 (min (width - thumb_width) 
          (fst pos - thumb_width / 2)) in
          let new_value = (new_thumb_x * 4) / (width - thumb_width) + 1 in
          slider_controller.change_value new_value (*updates value*)
        | _ -> ()
      );
  ...
  (* Creating a label widget with s as the label string *)
  let (w_label, _) = label s in
  
  (* Creating a combined widget of the slider and label *)
  let combined_widget = vpair w_label slider_widget in

  (combined_widget, slider_controller)
